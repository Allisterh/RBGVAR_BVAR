BVAR_irf_chol = function(bvar, hor = 36, shock = 1, pcol=2, normalised = NULL, sign = 1){
  ...
  # hor = 36; shock = 1;draws = 1000;i=1;j=1; pcol=2;normalised=1; sign =1
  CM = bvar$CM
  Sigma = bvar$Sigma
  n = bvar$vardata$number_of_endogenous
  draws = bvar$draws
  ...
  for (i in 1:n){
    ...
    names(irf_M)[i] = paste(bvar$vardata$names_of_endog_variables[i],",M", sep = "")
    
    irf_U[,i] = as.matrix(apply(irf[1:hor,i,shock,1:draws], 1, "quantU"))
    names(irf_U)[i] = paste(bvar$vardata$names_of_endog_variables[i],",U", sep = "")
    
    irf_L[,i] = as.matrix(apply(irf[1:hor,i,shock,1:draws], 1, "quantL"))
    names(irf_L)[i] = paste(bvar$vardata$names_of_endog_variables[i],",L", sep = "")
  }
